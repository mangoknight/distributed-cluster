 ...
 *     > gear.conf         — app.conf
 *     > geardefault.conf  — default.conf
 *     > reference.conf
 * }}}
  * withFallback(user).withFallback(gear).withFallback(gearDefault) 向后覆盖
  *
  * https://github.com/lightbend/config
  * 1、users can override the config with Java system properties, java -Dmyapp.foo.bar=10
  * 可以以-Dgearpump.config.file/resource指定配置文件也可以以-Dmyapp.foo.bar指定单独配置项(...)
  * 2、This means roughly that the default is to parse all application.conf, application.json and application.properties found at the root of the class path
  *
 */
...
object ClusterConfig {
  ...
  private[gearpump] def load(source: ClusterConfigSource): Configs = {
    ...
    val gear = ConfigFactory.parseResourcesAnySyntax("app.conf", // gear
      ConfigParseOptions.defaults.setAllowMissing(true))

    val gearDefault = ConfigFactory.parseResourcesAnySyntax("default.conf", // geardefault
      ConfigParseOptions.defaults.setAllowMissing(true))
  ...
  private def replaceHost(config: Config): Config = {
    val hostName = config.getString(Constants.HOSTNAME)
