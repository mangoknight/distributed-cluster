...
object LogUtil {
  ...
  def loadConfiguration(config: Config, processType: ProcessType.ProcessType): Unit = {
    // Set log file name
    val propName = s"rs.${processType.toString.toLowerCase}.log.file"
    val props = loadConfiguration

    props.setProperty("rs.log.file", "${" + propName + "}")

    props.setProperty("JVM_NAME", jvmName)

    processType match {
      case ProcessType.APPLICATION =>
        props.setProperty("log4j.rootAppender", "${rsS.application.logger}")
        props.setProperty("rs.application.log.rootdir",
          applicationLogDir(config).getAbsolutePath)
      case _ =>
        props.setProperty("log4j.rootAppender", "${rs.root.logger}")
        props.setProperty("rs.log.dir", daemonLogDir(config).getAbsolutePath)
    }
  ...
  def daemonLogDir(config: Config): File = {
    Util.asSubDirOfGearpumpHome(config.getString(Constants.LOG_DAEMON_DIR))
  }
  ...
  def applicationLogDir(config: Config): File = {
    Util.asSubDirOfGearpumpHome(config.getString(Constants.LOG_APPLICATION_DIR))
  }
}