...
package com.mango.cluster.ddata
...
class InMemoryKVService extends Actor with Stash {
  ...
  private val readAll = ReadAll(timeout)
  private val writeAll = WriteAll(timeout)
  ...
  def kvService: Receive = {
    ...
    case GetKV(group: String, key: String ) =>
      ...
      replicator ! Get(groupKey(group), readAll, Some(request))
    ...
    case PutKV(group: String, key: String, value: Any) =>
      val request = Request(sender(), key)
      val update = Update(groupKey(group), LWWMap(), writeAll, Some(request)) { map =>
        map + (key -> value)
      }
      replicator ! update
    ...
    case DeleteKVGroup(group: String) =>
      replicator ! Delete(groupKey(group), writeAll)
